...
class AgentCheck(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.hostname = "localhost"  # `self.hostname` is deprecated, use `datadog_agent.get_hostname()` instead
        ...
        for key, value in event.items():
            ...
            if isinstance(value, unicode):
                try:
                    ...
                    self.log.warning(
                        "Error encoding unicode field '%s' of event to utf-8 encoded string, can't submit event", key)
                    return
        if event.get('tags'):
